...
export class SwipeRepository {
  ...
  async recordSwipe(
    ...
    const { error } = await this.supabase.from("swipes").upsert(
      {
        user_id: userId,
        film_id: filmId,
        direction,
  ...
  async upsertAttributeWeight(
    userId: string,
    attributeType: string,
    attributeValue: string,
    weight: number,
  ): Promise<void> {
    const { error } = await this.supabase.from("user_taste_profile").upsert(
      {
        user_id: userId,
  ...
  async getUserGenreWeights(
    userId: string,
  ): Promise<{ attribute_value: string; weight: number }[]> {
    const { data, error } = await this.supabase
      .from("user_taste_profile")
  ...
  async getUserPeopleWeights(
    userId: string,
  ): Promise<
    { attribute_type: string; attribute_value: string; weight: number }[]
  > {
    const { data, error } = await this.supabase
      .from("user_taste_profile")
  ...
  async getSwipedExternalFilmIds(userId: string): Promise<Set<number>> {
    ...
    for (const row of data ?? []) {
      const films = row.films as unknown as
        | { external_film_id: number }
        | { external_film_id: number }[]
        | null;
      const externalId = Array.isArray(films)
        ? films[0]?.external_film_id
        : films?.external_film_id;
  ...
  async getRecentLikedExternalFilmIds(
    userId: string,
    limit: number = 3,
  ): Promise<number[]> {
    const { data, error } = await this.supabase
